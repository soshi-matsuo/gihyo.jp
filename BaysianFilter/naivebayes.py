...
class NaiveBayes:
    def __init__(self):
        ...
        # {category1:{word1:n, word2:n,...}, category2:{...}}
        self.wordcount = {}
        # あるカテゴリが登場した回数
    ...
    def score(self, word, cat):
        # P(word|cat)すべての積がP(doc|cat)になるが、少数計算を繰り返すことでアンダーフローの恐れがある
        # よって、対数化して積の計算を和の計算に変換する
...
if __name__ == "__main__":
    ...
    Unicodeによる文字列操作をサポートしており，日本語処理も標準で可能である。多くのプラットフォームをサポートしており（動作するプラットフォーム）
    また，豊富なドキュメント，豊富なライブラリがあることから，産業界でも利用が増えつつある。''', 'Python')
    ...
    それらの手法は， Machine Learning などの学術雑誌などで発表されることが多い。''', u'機械学習')
